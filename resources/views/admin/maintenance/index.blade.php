        ...
        <div class="bg-white rounded-xl shadow-sm overflow-hidden mb-6">
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <h2 class="text-xl font-bold text-gray-900 flex items-center gap-3">
                            <i class="mdi mdi-wrench text-2xl {{ $maintenanceMode && $maintenanceMode->is_active ? 'text-orange-500' : 'text-green-500' }}"></i>
                            Gestion du Mode Maintenance
                        </h2>
                        <p class="text-sm text-gray-600 mt-1">Activer ou désactiver le mode maintenance pour l'application</p>
                    </div>
                    @if($maintenanceMode && $maintenanceMode->is_active)
                ...
                <div class="p-6 bg-gray-50 border-b border-gray-100">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="text-sm font-semibold {{ $maintenanceMode->is_active ? 'text-orange-600' : 'text-green-600' }}">
                                {{ $maintenanceMode->is_active ? 'En maintenance' : 'Disponible' }}
                            </p>
                        </div>
                        @if($maintenanceMode->is_active && $maintenanceMode->activated_at)
                            <div>
                                <p class="text-xs text-gray-500 font-medium uppercase mb-1">Activé le</p>
                                <p class="text-sm text-gray-900">{{ $maintenanceMode->activated_at->format('d/m/Y à H:i') }}</p>
                            </div>
                        @elseif(!$maintenanceMode->is_active && $maintenanceMode->deactivated_at)
                            <div>
                                <p class="text-xs text-gray-500 font-medium uppercase mb-1">Désactivé le</p>
                                <p class="text-sm text-gray-900">{{ $maintenanceMode->deactivated_at->format('d/m/Y à H:i') }}</p>
                            </div>
                        @endif
                        @if($maintenanceMode->activatedBy)
                            <div>
                                <p class="text-xs text-gray-500 font-medium uppercase mb-1">Modifié par</p>
                                <p class="text-sm text-gray-900">{{ $maintenanceMode->activatedBy->name }}</p>
                            </div>
                        @endif
                        @if($maintenanceMode->message)
                            <div class="md:col-span-2">
                                <p class="text-xs text-gray-500 font-medium uppercase mb-1">Message actuel</p>
                                <p class="text-sm text-gray-900">{{ $maintenanceMode->message }}</p>
                            </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="p-6">
                <form action="{{ route('admin.maintenance.toggle') }}" method="POST" id="maintenanceForm">
                        ...
                        <label class="flex items-center justify-between cursor-pointer group">
                            ...
                            <div class="relative ml-4">
                                <input type="checkbox"
                                       ...
                                       id="is_active"
                                       value="1"
                                       {{ $maintenanceMode && $maintenanceMode->is_active ? 'checked' : '' }}
                                       class="sr-only peer"
                    ...
                    <!-- Message Input -->
                    <div class="mb-6" id="messageContainer" style="display: {{ $maintenanceMode && $maintenanceMode->is_active ? 'block' : 'none' }}">
                        ...
                        >{{ old('message', $maintenanceMode ? $maintenanceMode->message : '') }}</textarea>
            ...
            <div class="bg-white rounded-xl shadow-sm overflow-hidden">
                ...
                <div class="p-6">
                    <div class="flex items-start gap-4">
                        <div class="w-10 h-10 bg-gradient-to-br {{ $maintenanceMode->is_active ? 'from-orange-500 to-red-500' : 'from-green-500 to-teal-500' }} rounded-full flex items-center justify-center flex-shrink-0">
                            <i class="mdi {{ $maintenanceMode->is_active ? 'mdi-wrench' : 'mdi-check' }} text-white text-xl"></i>
                        </div>
                        <div class="flex-1">
                            <p class="text-sm font-semibold text-gray-900">
                                {{ $maintenanceMode->is_active ? 'Mode maintenance activé' : 'Mode maintenance désactivé' }}
                            </p>
                            <p class="text-xs text-gray-500 mt-1">
                                {{ $maintenanceMode->updated_at->diffForHumans() }}
                                @if($maintenanceMode->activatedBy)
                                    par {{ $maintenanceMode->activatedBy->name }}
                                @endif
                            </p>
                            @if($maintenanceMode->message)
