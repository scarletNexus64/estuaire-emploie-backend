...
@section('page-title', 'Gérer les Étapes du Programme')
...
<div class="row">
    <div class="col-lg-8">
        ...
        <div class="card mb-3" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white;">
            <div style="padding: 1.5rem; display: flex; align-items: center; gap: 1rem;">
                ...
                <div>
                    <h5 class="mb-1" style="color: white;">{{ $program->title }}</h5>
                    <p class="mb-0" style="opacity: 0.9;">{{ $program->steps->count() }} étape(s) configurée(s)</p>
                </div>
            </div>
        </div>
        ...
        <div class="card">
            ...
            <div style="padding: 1.5rem;">
                @forelse($program->steps as $index => $step)
                <div class="card mb-3" style="border-left: 4px solid var(--primary);" id="step-{{ $step->id }}">
                    <div style="padding: 1rem;">
                        <div style="display: flex; justify-content: space-between; align-items-start;">
                            <div style="flex: 1;">
                                ...
                                @if($step->content)
                                <div class="mb-2">
                                    <button class="btn btn-sm btn-outline-secondary" onclick="toggleContent('content-{{ $step->id }}')">
                                    ...
                                    <div id="content-{{ $step->id }}" style="display: none; margin-top: 0.75rem; padding: 1rem; background: var(--light); border-radius: 8px;">
                                        <div style="white-space: pre-line;">{{ $step->content }}</div>
                                    </div>
                                </div>
                                @endif
                            ...
                            <div style="display: flex; gap: 0.5rem;">
                                <button class="btn btn-sm btn-secondary" onclick="editStep({{ $step->id }})" title="Modifier">
                                    <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                    </svg>
                                </button>
                                <form method="POST" action="{{ route('admin.programs.destroy-step', [$program, $step]) }}" style="display: inline;">
                                    @csrf
                                    @method('DELETE')
    ...
    <div class="col-lg-4">
        ...
        <div class="card">
            ...
            <div style="padding: 1.5rem;">
                <form method="POST" action="{{ route('admin.programs.store-step', $program) }}" id="stepForm">
                    @csrf
                        ...
                        <div class="col-6">
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control"
                                       id="order" name="order" value="{{ $program->steps->max('order') + 1 }}" min="0">
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="form-group">
                    ...
                    <a href="{{ route('admin.programs.show', $program) }}" class="btn btn-outline-secondary mt-2" style="width: 100%;">
                        Retour au Programme
                    </a>
                </form>
            </div>
...
function resetForm() {
    ...
    document.getElementById('stepForm').action = "{{ route('admin.programs.store-step', $program) }}";
...
async function editStep(stepId) {
    try {
        ...
        const step = await new Promise((resolve, reject) => {
            ...
            xhr.open('GET', `/admin/programs/{{ $program->id }}/steps/${stepId}`);
        ...
        document.getElementById('stepForm').action = `/admin/programs/{{ $program->id }}/steps/${stepId}`;
