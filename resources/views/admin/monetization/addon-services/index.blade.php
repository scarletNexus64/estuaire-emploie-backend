...
@section('page-title', 'Services Additionnels Recruteurs')
...
@section('header-actions')
    <a href="{{ route('admin.addon-services.create') }}" class="header-btn">
...
<div class="card">
    ...
    @if($services->isEmpty())
        <div style="text-align: center; padding: 4rem 2rem;">
            ...
            <a href="{{ route('admin.addon-services.create') }}" class="btn btn-primary">
                ...
                <tbody>
                    @foreach($services as $service)
                        <tr>
                            ...
                            <td>
                                <strong>{{ $service->companyServices->count() }}</strong>
                            </td>
                            <td>
                                <div style="display: flex; gap: 0.5rem;">
                                    <a href="{{ route('admin.addon-services.edit', $service->id) }}" class="btn btn-sm btn-primary">
                                        <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                        </svg>
                                    </a>
                                    <form action="{{ route('admin.addon-services.toggle', $service->id) }}" method="POST" style="display: inline;">
                                        @csrf
                                        @method('PATCH')
                                    ...
                                    <form action="{{ route('admin.addon-services.destroy', $service->id) }}" method="POST" style="display: inline;" onsubmit="return confirm('Supprimer ce service ?');">
                                        @csrf
                                        @method('DELETE')
