...
@section('breadcrumbs')
    <span>/ <a href="{{ route('admin.addon-services.index') }}">Services Additionnels</a> / {{ $isEdit ? 'Modifier' : 'Créer' }}</span>
@endsection

@section('content')
<form action="{{ $isEdit ? route('admin.addon-services.update', $service->id) : route('admin.addon-services.store') }}" method="POST">
    @csrf
        ...
        <div>
            ...
            <div class="card" style="margin-top: 1.5rem;">
                <div class="card-body" style="display: flex; gap: 0.75rem;">
                    ...
                    <a href="{{ route('admin.addon-services.index') }}" class="btn btn-secondary">Annuler</a>
                </div>
            </div>
        </div>
    </div>
