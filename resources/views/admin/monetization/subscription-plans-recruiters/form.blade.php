...
@section('breadcrumbs')
    <span>/ <a href="{{ route('admin.subscription-plans.recruiters.index') }}">Plans & Tarifs</a> / {{ $isEdit ? 'Modifier' : 'Créer' }}</span>
@endsection

@section('content')
<form action="{{ $isEdit ? route('admin.subscription-plans.recruiters.update', $plan->id) : route('admin.subscription-plans.recruiters.store') }}" method="POST">
    @csrf
    @if($isEdit)
        @method('PUT')
    @endif
        ...
        <div>
            ...
            <div class="card" style="margin-top: 1.5rem;">
                <div class="card-body" style="display: flex; gap: 0.75rem;">
                    ...
                    <a href="{{ route('admin.subscription-plans.recruiters.index') }}" class="btn btn-secondary">
                        Annuler
                    </a>
                </div>
            </div>
