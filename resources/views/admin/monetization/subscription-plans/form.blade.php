...
@section('breadcrumbs')
    <span>/ <a href="{{ route('admin.subscription-plans.index') }}">Plans & Tarifs</a> / {{ $isEdit ? 'Modifier' : 'Créer' }}</span>
@endsection

@section('content')
<form action="{{ $isEdit ? route('admin.subscription-plans.update', $plan->id) : route('admin.subscription-plans.store') }}" method="POST">
    @csrf
    @if($isEdit)
        @method('PUT')
    @endif
