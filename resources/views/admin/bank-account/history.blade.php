...
<div class="container-fluid py-4">
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <h5 class="mb-0 fw-bold">{{ $withdrawals->where('status', 'completed')->count() }}</h5>
                        </div>
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <h5 class="mb-0 fw-bold">{{ $withdrawals->where('status', 'pending')->count() }}</h5>
                        </div>
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <h5 class="mb-0 fw-bold">{{ $withdrawals->where('status', 'failed')->count() }}</h5>
                        </div>
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <h5 class="mb-0 fw-bold">{{ $withdrawals->total() }}</h5>
                        </div>
    ...
    <div class="card border-0 shadow-sm mb-4">
        ...
        <div class="card-body">
            <form method="GET" action="{{ route('admin.bank-account.history') }}" class="row g-3" id="filterForm">
                <div class="col-md-3">
                ...
                <div class="col-md-2">
                    ...
                    <select name="payment_method" id="payment_method" class="form-select">
                        ...
                        <option value="momo" {{ request('payment_method') === 'momo' ? 'selected' : '' }}>MTN MoMo</option>
                    </select>
                </div>
                ...
                <div class="col-md-3 d-flex align-items-end gap-2">
                    ...
                    <a href="{{ route('admin.bank-account.history') }}" class="btn btn-outline-secondary">
            ...
            <div class="d-flex justify-content-between align-items-center">
                <h5 class="mb-0">
                    ...
                    @if($withdrawals->total() > 0)
                        <span class="badge bg-primary ms-2">{{ $withdrawals->total() }}</span>
                    @endif
                </h5>
            ...
            <div class="table-responsive">
                <table class="table table-hover mb-0" id="withdrawalsTable">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><strong class="text-primary">#{{ $withdrawal['id'] }}</strong></td>
                            <td>
                            ...
                            <td>
                                ...
                                @endphp
                                <span class="text-{{ $fees > 0 ? 'danger' : 'muted' }}">
                                    {{ $fees > 0 ? '-' : '' }}{{ number_format($fees, 0, ',', ' ') }}
                                </span>
                            </td>
                            <td>
                                <strong class="text-success">{{ number_format($withdrawal['amount_sent'], 0, ',', ' ') }}</strong>
                            </td>
                            <td>
                            ...
                            <td>
                                <code class="small">{{ $withdrawal['payment_account'] }}</code>
                            </td>
                            <td>
                                <div class="d-flex align-items-center">
                                    <i class="fas fa-user-shield text-muted me-1"></i>
                                    <span class="small">{{ $withdrawal['admin_name'] }}</span>
                                </div>
                            </td>
                            <td>
                            ...
                            <td>
                                @if($withdrawal['freemopay_reference'])
                                    <code class="small text-primary">{{ $withdrawal['freemopay_reference'] }}</code>
                                @else
                            ...
                            <td class="text-center">
                                <div class="btn-group btn-group-sm" role="group">
                                    <button class="btn btn-outline-primary" onclick="viewDetails({{ json_encode($withdrawal) }})" title="Voir détails">
                                        <i class="fas fa-eye"></i>
                                    </button>
                                    @if($withdrawal['admin_notes'])
                                    <button class="btn btn-outline-info" onclick="viewNotes('{{ addslashes($withdrawal['admin_notes']) }}')" title="Voir notes">
                    ...
                    <tfoot class="table-light">
                        <tr>
                            ...
                            <td class="fw-bold">
                                {{ number_format($withdrawals->sum('amount'), 0, ',', ' ') }} XAF
                            </td>
                            <td colspan="10"></td>
                        </tr>
                    </tfoot>
            ...
            <div class="card-footer bg-white border-0">
                <div class="d-flex justify-content-between align-items-center">
                    <div class="text-muted small">
                        Affichage de {{ $withdrawals->firstItem() ?? 0 }} à {{ $withdrawals->lastItem() ?? 0 }} sur {{ $withdrawals->total() }} retrait(s)
                    </div>
                    {{ $withdrawals->links() }}
                </div>
            </div>
