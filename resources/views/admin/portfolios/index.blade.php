                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="search" name="search"
                               value="{{ request('search') }}" placeholder="Nom, email, titre, slug...">
                    </div>
                </div>
            ...
            <tbody>
                ...
                <tr>
                    <td class="checkbox-cell">
                        <input type="checkbox" class="custom-checkbox row-checkbox" value="{{ $portfolio->id }}">
                    </td>
                    <td>
                        <div style="display: flex; align-items: center; gap: 10px;">
                            @if($portfolio->photo_url)
                                <img src="{{ $portfolio->photo_url }}" alt="Photo" style="width: 40px; height: 40px; border-radius: 50%; object-fit: cover;">
                            @else
                            ...
                            <div>
                                ...
                                <div class="small text-muted">{{ $portfolio->user->email }}</div>
                            </div>
                        </div>
                    </td>
                    <td><strong>{{ $portfolio->title }}</strong></td>
                    <td>
                        <code style="background: var(--light); padding: 0.25rem 0.5rem; border-radius: 4px;">{{ $portfolio->slug }}</code>
                    </td>
                    <td>
                        @php
                            $templateBadges = [
                        ...
                        @endphp
                        <span class="badge badge-{{ $templateBadges[$portfolio->template_id] ?? 'secondary' }}">
                    ...
                    <td>
                        <strong>{{ number_format($portfolio->view_count) }}</strong>
                    </td>
                    <td>{{ $portfolio->created_at->format('d/m/Y') }}</td>
                    <td>
                        <div style="display: flex; gap: 0.5rem;">
                            <a href="{{ $portfolio->public_url }}" target="_blank" class="btn btn-sm btn-info" title="Voir le portfolio">
                                <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/>
                                </svg>
                            </a>

                            <a href="{{ route('admin.portfolios.show', $portfolio) }}" class="btn btn-sm btn-primary" title="Détails & Stats">
                                <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/>
                                </svg>
                            </a>

                            <form method="POST" action="{{ route('admin.portfolios.toggle-visibility', $portfolio) }}" style="display: inline;">
                                @csrf
                                @method('PATCH')
...
<!-- Bulk Delete Form -->
<form id="bulkDeleteForm" method="POST" action="{{ route('admin.portfolios.bulk-delete') }}" style="display: none;">
    @csrf
    @method('DELETE')
</form>
@endsection
