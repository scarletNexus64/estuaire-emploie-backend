<div class="tab-pane fade" id="paypal" role="tabpanel" aria-labelledby="paypal-tab">
    <div class="row">
        <div class="col-lg-8">
            <form action="{{ route('admin.service-config.update-paypal') }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('paypal_mode') is-invalid @enderror"
                        ...
                        <option value="sandbox" {{ old('paypal_mode', $config?->paypal_mode ?? '') == 'sandbox' ? 'selected' : '' }}>
                            Sandbox (Test)
                        </option>
                        <option value="live" {{ old('paypal_mode', $config?->paypal_mode ?? '') == 'live' ? 'selected' : '' }}>
                            Live (Production)
                        </option>
                    </select>
                    @error('paypal_mode')
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <div class="input-group">
                            <input type="password" class="form-control @error('paypal_client_id') is-invalid @enderror"
                                   id="paypal_client_id" name="paypal_client_id"
                                   value="{{ old('paypal_client_id', $config?->paypal_client_id ?? '') }}"
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <div class="input-group">
                            <input type="password" class="form-control @error('paypal_client_secret') is-invalid @enderror"
                                   ...
                                   value="{{ old('paypal_client_secret', $config?->paypal_client_secret ?? '') }}"
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('paypal_currency') is-invalid @enderror"
                            id="paypal_currency" name="paypal_currency" required>
                        <option value="USD" {{ old('paypal_currency', $config?->paypal_currency ?? 'USD') == 'USD' ? 'selected' : '' }}>USD - Dollar américain</option>
                        <option value="EUR" {{ old('paypal_currency', $config?->paypal_currency ?? '') == 'EUR' ? 'selected' : '' }}>EUR - Euro</option>
                        <option value="XAF" {{ old('paypal_currency', $config?->paypal_currency ?? '') == 'XAF' ? 'selected' : '' }}>XAF - Franc CFA</option>
                        <option value="GBP" {{ old('paypal_currency', $config?->paypal_currency ?? '') == 'GBP' ? 'selected' : '' }}>GBP - Livre sterling</option>
                        <option value="CAD" {{ old('paypal_currency', $config?->paypal_currency ?? '') == 'CAD' ? 'selected' : '' }}>CAD - Dollar canadien</option>
                    </select>
                ...
                <div class="d-flex justify-content-between">
                    ...
                    <button type="button" class="btn btn-info" onclick="testService('PayPal', '{{ route('admin.service-config.test-paypal') }}')">
